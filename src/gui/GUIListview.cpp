...
GUIListviewColumn::GUIListviewColumn():
	GUIListviewColumn("Kein Text") {
}

GUIListviewColumn::GUIListviewColumn(const std::string& text) {
	text_ = text;
...
GUITexture* GUIListviewColumn::GetTexture(GUIRenderer* renderer,TTF_Font*& font, TTF_Font*& smallFont, SDL_Color color, GUITexture** textureTextDetails, const int maxWith) {
...
GUIListview::GUIListview(const GUIPoint position, const GUISize size, const std::string& name, const SDL_Color background, const SDL_Color textcolor) :
	...
	buttonDownEvent_ = std::bind(&GUIListview::ButtonDown, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3);
